...
export async function upsertEnrichment(
  ...
  if (existing.length > 0) {
    ...
    db.run(
      `INSERT INTO alert_enrichments (alert_id, ai_analysis, ioc_type, threat_intel, recommendation, confidence, related_cves, geo_country, geo_city, asn_info, llm_provider, llm_model)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        alertId,
